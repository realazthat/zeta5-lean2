...
theorem outerResidueMass_scale (k K N p : ℚ) (hk : 0<k) :
  ...
  have h1 : k*K<k*p ↔ K<p := by constructor <;> intro h <;> nlinarith
  have h2 : k*K<2*(k*p) ↔ K<2*p := by constructor <;> intro h <;> nlinarith
  unfold outerResidueMass
  simp only [h1,h2]
  split_ifs with hK htwo
  · simp
  · rw [show k*K-k*p=k*(K-p) by ring,
      show k*N+k*K-2*(k*p)=k*(N+K-2*p) by ring,
      show k*K+4*(k*N)-2*(k*p)=k*(K+4*N-2*p) by ring,
      ← mul_min_of_nonneg N (K-p) hk.le]
    rw [show max (k*(N+K-2*p)) 0=k*max (N+K-2*p) 0 by
      rw [mul_max_of_nonneg _ _ hk.le,mul_zero]]
    rw [show max (k*(K+4*N-2*p)) 0=k*max (K+4*N-2*p) 0 by
      rw [mul_max_of_nonneg _ _ hk.le,mul_zero]]
    ring
  · rw [show k*K-2*(k*p)=k*(K-2*p) by ring,
      show k*N+k*K-3*(k*p)=k*(N+K-3*p) by ring,
      ← mul_min_of_nonneg N (K-2*p) hk.le]
    rw [show max (k*(N+K-3*p)) 0=k*max (N+K-3*p) 0 by
      rw [mul_max_of_nonneg _ _ hk.le,mul_zero]]
    ring
